...
- [ ] **Sessão de restrições no brain (o que a Lena NÃO pode falar).** Adicionar campo `forbidden_topics` ou bloco específico no `tenant_brains.escalation_rules` que liste por tenant: temas proibidos (ex.: diagnóstico médico, conselhos jurídicos vinculantes, garantias de resultado em estética, valores não publicados), palavras-gatilho que disparam handoff imediato (ex.: "reclamação", "advogado", "Procon"), e fórmulas obrigatórias de aviso ("posso te encaminhar para nosso atendimento humano"). Refletir no system prompt como bloco "RESTRIÇÕES OBRIGATÓRIAS". UI na Central → Cérebro → nova aba "Restrições".
- [ ] **Política de identificação pessoal no atendimento (público vs privado).** Definir contratualmente e tecnicamente o que pode aparecer nas mensagens da Lena: nomes de profissionais do estabelecimento ("com a Dra. Helena"), nomes de operadores que assumiram a conversa ("vou te conectar com a Maria"), telefones/e-mails internos. Responsabilidade do contratante: ele indica o que é público. Schema: campo `public_profiles` no `tenant_brains` listando nomes/funções autorizadas; o resto fica em campo separado privado que a Lena conhece mas não cita. Reflete na LGPD (consentimento + minimização). UI na Central → Cérebro → "Equipe pública" e "Equipe interna".
