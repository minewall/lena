...
import { describeTone } from "../tones/index.ts";
...
function buildProductsBlock(cfg: TenantBrain): string {
  const products = (cfg.products ?? []).filter((p) => p && String(p.name || "").trim());
  if (products.length === 0) return "";
  const lines = products.map((p) => {
    const price = String(p.price || "").trim() || "sob consulta";
    let av = "";
    if (p.tracked) {
      av = (p.stock ?? 0) <= 0 ? " (esgotado)" : ` (${p.stock} em estoque)`;
    }
    return `- ${p.name} (${price})${av}`;
  });
  return `\n\nPRODUTOS À VENDA.\nVocê também vende produtos. Ofereça quando fizer sentido e confira o estoque antes de fechar:\n${lines.join("\n")}\nNUNCA venda um produto marcado como esgotado. Se acabou, ofereça avisar quando voltar ou sugira uma alternativa.`;
}
...
export function buildDemoSystem(cfg: TenantBrain): string {
  ...
  const productsBlock = buildProductsBlock(cfg);
...
• Só envolva um humano em casos fora do alcance (reclamações sérias, situações sensíveis, questões médicas ou clínicas, pagamentos com problema).${prepBlock}${sessionsBlock}${productsBlock}${combosBlock}${paymentsBlock}${locationBlock}${restrictionsBlock}${teamBlock}`;
}
